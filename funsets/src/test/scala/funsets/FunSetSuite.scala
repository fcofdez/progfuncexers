...
class FunSetSuite extends FunSuite {
  ...
  */
  ...
  test("singletonSet(1) contain not 15") {
    new TestSets {
      assert(!contains(s1, 15), "Singleton")
    }
  }
  ...
  test("intersect contains all elements") {
    new TestSets {
      val s = intersect(positive, lessThan5)
        assert(contains(s, 1), "intersect 1")
      assert(!contains(s, 0), "intersect 2")
      assert(!contains(s, -1), "intersect 3")
      assert(!contains(s, 5), "intersect 4")
  ...
  test("forall") {
    new TestSets {
      ...
      assert(forall(s1, s1))
      assert(forall(positive, x => x > -100))
      assert(!forall(positive, x => x > 100))
    }
  }
  ...
  test("exists") {
    new TestSets {
      ...
      assert(!exists(positive, x => x == -100))
    }
  }
